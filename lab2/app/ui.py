...
class SettingsFrame(tk.Toplevel):
    ...
    def create_widgets(self):
        tk.Label(self, text="Максимальное количество новых токенов:").grid(row=0, column=0, padx=10, pady=5, sticky=tk.W)
        self.max_new_tokens_var = tk.IntVar(value=self.inference.max_new_tokens)
        tk.Entry(self, textvariable=self.max_new_tokens_var).grid(row=0, column=1, padx=10, pady=5)

        tk.Label(self, text="No repeat n-gram size:").grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)
        self.no_repeat_ngram_size_var = tk.IntVar(value=self.inference.no_repeat_ngram_size)
        tk.Entry(self, textvariable=self.no_repeat_ngram_size_var).grid(row=1, column=1, padx=10, pady=5)

        tk.Label(self, text="Repetition penalty:").grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
        self.repetition_penalty_var = tk.DoubleVar(value=self.inference.repetition_penalty)
        tk.Entry(self, textvariable=self.repetition_penalty_var).grid(row=2, column=1, padx=10, pady=5)

        tk.Label(self, text="Температура:").grid(row=3, column=0, padx=10, pady=5, sticky=tk.W)
        self.temperature_var = tk.DoubleVar(value=self.inference.temperature)
        tk.Entry(self, textvariable=self.temperature_var).grid(row=3, column=1, padx=10, pady=5)

        tk.Button(self, text="Применить", command=self.apply_settings).grid(row=4, column=0, columnspan=2, pady=10)
...
class MainFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.model_output.grid(row=0, columnspan=3, sticky=tk.NSEW)
