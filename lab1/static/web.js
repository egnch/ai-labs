...
;
	($.$labs_app) = class $labs_app extends ($.$mol_book2) {
		original_text(next){
			if(next !== undefined) return next;
			return "";
		}
		Original_text(){
			...
			(obj.hint) = () => ("Введите текст");
			(obj.value) = (next) => ((this?.original_text(next)));
			return obj;
		}
		Translated_text(){
			const obj = new this.$.$mol_list();
			(obj.rows) = () => ((this?.translated_list()));
			return obj;
		}
		Minor_button_icon(){
			const obj = new this.$.$mol_icon_book_refresh();
			return obj;
		}
		Random_button(){
			const obj = new this.$.$mol_button_minor();
			(obj.click) = (next) => ((this?.set_random_text(next)));
			(obj.sub) = () => ([(this?.Minor_button_icon())]);
			return obj;
		}
		Main_page(){
			const obj = new this.$.$mol_page();
			(obj.title) = () => ("Лабораторная работа №1");
			(obj.body) = () => ([(this?.Original_text()), (this?.Translated_text())]);
			(obj.tools) = () => ([(this?.Random_button())]);
			return obj;
		}
		Result_text(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ((this?.statistics_text()));
			return obj;
		}
		Result_page(){
			const obj = new this.$.$mol_page();
		...
		Translated_card(id){
			...
			(obj.content) = () => ((this?.translated_text(id)));
			return obj;
		}
		choose_best(id){
			return null;
		...
		Translated_button(id){
			const obj = new this.$.$mol_button_minor();
			(obj.click) = (next) => ((this?.choose_best(id)));
			(obj.title) = () => ("Выбрать");
			(obj.disabled) = () => ((this?.was_selected()));
			return obj;
		}
		pages(){
			return [(this?.Main_page()), (this?.Result_page())];
		}
		Translated(id){
			const obj = new this.$.$mol_row();
			(obj.sub) = () => ([(this?.Translated_card(id)), (this?.Translated_button(id))]);
			return obj;
		}
	};
	($mol_mem(($.$labs_app.prototype), "original_text"));
	($mol_mem(($.$labs_app.prototype), "Original_text"));
	($mol_mem(($.$labs_app.prototype), "Translated_text"));
	($mol_mem(($.$labs_app.prototype), "Minor_button_icon"));
	($mol_mem(($.$labs_app.prototype), "Random_button"));
	($mol_mem(($.$labs_app.prototype), "Main_page"));
	($mol_mem(($.$labs_app.prototype), "Result_text"));
	($mol_mem(($.$labs_app.prototype), "Result_page"));
	($mol_mem_key(($.$labs_app.prototype), "Translated_card"));
	($mol_mem_key(($.$labs_app.prototype), "Translated_button"));
	($mol_mem_key(($.$labs_app.prototype), "Translated"));
...
var $;
(function ($) {
    $.$labs_translation_api_url = "/api/translate";
    $.$labs_translation_json = $mol_data_record({
        translation: $mol_data_string,
    });
})($ || ($ = {}));
...
(function ($) {
    ...
    (function ($$) {
        ...
        (function (TranslationService) {
            TranslationService["deep"] = "deep-translate";
            TranslationService["google"] = "google-translate";
            TranslationService["yandex"] = "yandex-translate";
            TranslationService["gpt_4o"] = "openai-gpt-4o";
            TranslationService["gpt_4o_mini"] = "openai-gpt-4o-mini";
            TranslationService["gpt_3_5_turbo"] = "openai-gpt-3.5-turbo";
        })(TranslationService || (TranslationService = {}));
        class $labs_app extends $.$labs_app {
            statistics(next) {
                return next || {
                    "deep-translate": 0,
                    "google-translate": 0,
            ...
            statistics_text() {
                let total = Object.values(this.statistics()).reduce((sum, count) => sum + count, 0) || 1;
                let text = "# Статистика\n";
                text += "| Сервис | Счет | Результат |\n";
                text += Object.entries(this.statistics()).map(([service, count]) => {
                    return `| ${service} | ${count} | ${(count / total).toFixed(2)} |`;
                }).join("\n");
                return text;
            }
            was_selected(next) {
            ...
            translations() {
                ...
                const translations = $mol_wire_sync(this.make_translations).call(this, this.original_text());
                this.was_selected(false);
                return translations;
            }
            async make_translations(text) {
                const translations = await Promise.all(Object.values(TranslationService).map(async (service) => {
                    return [service, await this.translate(text, service)];
                }));
                return translations.reduce((res, [service, translation]) => {
                    res[service] = translation;
                    return res;
                }, {});
            }
            async translate(text, service) {
                const res = await fetch($labs_translation_api_url, {
                    method: "POST",
                ...
                return $labs_translation_json(await res.json()).translation;
            }
            translated_list() {
                if (!this.original_text())
                    return [];
                return $mol_array_shuffle(Object.keys(this.translations()))
                    .map(service => this.Translated(service));
            }
            translated_text(id) {
            ...
            choose_best(id) {
                ...
                const best_translation = translations[id];
                const updated_statistics = { ...this.statistics() };
                for (let service of Object.keys(updated_statistics)) {
                    if (translations[service] === best_translation) {
                        updated_statistics[service]++;
                    }
                }
                this.statistics(updated_statistics);
                this.was_selected(true);
            }
            set_random_text() {
                this.original_text($mol_stub_code());
            }
        }
        __decorate([
            $mol_mem
        ], $labs_app.prototype, "statistics", null);
        __decorate([
            $mol_mem
        ], $labs_app.prototype, "statistics_text", null);
        __decorate([
            $mol_mem
        ], $labs_app.prototype, "was_selected", null);
        __decorate([
            $mol_mem
        ], $labs_app.prototype, "translations", null);
        __decorate([
            $mol_mem
        ], $labs_app.prototype, "translated_list", null);
        __decorate([
            $mol_action
        ], $labs_app.prototype, "choose_best", null);
        __decorate([
            $mol_action
        ], $labs_app.prototype, "set_random_text", null);
        $$.$labs_app = $labs_app;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
